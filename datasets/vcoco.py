...
class VCOCO(torch.utils.data.Dataset):
    ...
    def __init__(self, img_set, img_folder, anno_file, transforms, num_queries, args):
        ...
        if img_set == 'train':
            with open("/comp_robot/pose_data/trainval_synvcoco_human_only.json", 'r') as f:
                self.annotations_syn = json.load(f)
            self.annotations.extend(self.annotations_syn)
    ...
    def __getitem__(self, idx):
        ...
        if "Syn" in img_anno["file_name"]:
            img = Image.open("/comp_robot/pose_data/SynVCOCO_v2/" + img_anno['file_name']).convert('RGB')
        else:
