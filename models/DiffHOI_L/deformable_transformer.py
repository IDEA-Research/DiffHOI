...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8,
        ...
        self.pos_guided_embedd = nn.Embedding(int(self.num_queries / 2), d_model)
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, sd_decoder, attn_mask=None):
        """
        Input:
        ...
        if self.two_stage_type == 'standard':
            ...
            refpoint_embed = refpoint_embed[:, :, None].repeat(1, 1, 2, 1)
            refpoint_embed = refpoint_embed.view(refpoint_embed.shape[0], -1, refpoint_embed.shape[-1])
            tgt = tgt.view(tgt.shape[0], int(self.num_queries / 2), 2, self.d_model) + self.pos_guided_embedd.weight[
                                                                                       None, :, None, :]
        ...
        # Begin Decoder
        #########################################################
        hs, hs_interaction, references = self.decoder(
            ...
            level_start_index=level_start_index,
            spatial_shapes=spatial_shapes,
            valid_ratios=valid_ratios, tgt_mask=attn_mask, sd_decoder=sd_decoder)
        #########################################################
        # End Decoder
        ...
        return hs, hs_interaction, references, hs_enc, ref_enc, init_box_proposal
        # hs: (n_dec, bs, nq, d_model)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
                ...
                sd_decoder: Optional[Tensor] = None,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if layer_id < self.interaction_decoder:
                # iter update
                if self.bbox_embed is not None:
                    ...
                    if self.dec_layer_number is not None and layer_id != self.num_layers - 1:
                        ...
                        if nq_now != select_number:
                            ...
                            topk_proposals = torch.topk(class_unselected.max(-1)[0], select_number, dim=0)[
                                1]  # new_nq, bs
            ...
            if layer_id == self.interaction_decoder - 1:

                human_object_pos = new_reference_points.view(int(new_reference_points.shape[0] / 2), 2,
                                                             new_reference_points.shape[1],
                                                             new_reference_points.shape[-1])
                human_object_pos = box_cxcywh_to_xyxy(human_object_pos)  # 450 , 2 , bs , 4 (xyxy)

                human_object_pos_ = torch.cat([human_object_pos.permute(0, 2, 1, 3)[..., :2].min(dim=2).values,
                                               human_object_pos.permute(0, 2, 1, 3)[..., 2:].max(dim=2).values], dim=-1)

                reference_points = box_xyxy_to_cxcywh(human_object_pos_)

                output_ = self.norm(output).view(int(output.shape[0] / 2), 2, output.shape[1], output.shape[-1])

                output = output_.mean(dim=1)

                sd_feats = [sd_feat.flatten(-2, -1).permute(2, 0, 1) for sd_feat in sd_decoder]
                sd_feats = torch.cat(sd_feats, dim=0)

                memory = memory + self.laynorm(self.project(sd_feats))

            if layer_id > self.interaction_decoder - 1:
                intermediate_interaction.append(self.norm(output))
        ...
        return [
            ...
            [itm_out.transpose(0, 1) for itm_out in intermediate_interaction],
